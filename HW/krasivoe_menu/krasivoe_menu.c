...
void GenerateMass(int size, int* mass) {
    ...
    printf("Ваш массив сгенерирован.\n");
...
void BubbleSort(int size, int* mass) {
    int i, j, imp;
    for (i = 0; i < size; i++) {
        for (j = i; j < size; j++) {
            if (mass[i] > mass[j]) {
    ...
    printf("Ваш массив отсортирован.\n");
    for (i = 0; i < size; i++) {
        printf("%d ", mass[i]);
    }
...
void SettingMenu(int size, int* mass) {
    ...
    int i;
    COORD cursorPos;
    char* menu_1[MENU_SIZE_1] = { "Задать массив", "Вывести текущий массив на экран", "Назад"};
    exit_flag_1 = 0;
    choose_pos_1 = 0;
    while (!exit_flag_1) {
        system("cls");
...
void AlgoritmMenu(int size, int* mass) {
    system("title Application");
    SetConsoleCP(1251);
    SetConsoleOutputCP(1251);
    HANDLE hStdOut = GetStdHandle(STD_OUTPUT_HANDLE);
    ...
    int i;
    COORD cursorPos;
    char* menu_2[MENU_SIZE_1] = { "Поиск", "Сортировка" };
    exit_flag_2 = 0;
    choose_pos_2 = 0;
    while (!exit_flag_2) {
        system("cls");
        ...
        cursorPos = (COORD){ 0, 0 };
        SetConsoleCursorPosition(hStdOut, cursorPos);
        while (iKey_2 != KEY_EXIT && iKey_2 != KEY_ENTER) {
            switch (iKey_2) {
            case KEY_ARROW_UP:
                choose_pos_2--;
                break;
            ...
            system("cls");
            if (choose_pos_2 < 0) { choose_pos_2 = MENU_SIZE_2 - 1; }
            if (choose_pos_2 > MENU_SIZE_2 - 1) { choose_pos_2 = 0; }
            ...
            cursorPos = (COORD){ 0, choose_pos_2 };
            SetConsoleCursorPosition(hStdOut, cursorPos);
            printf("<<", iKey_2);
            cursorPos = (COORD){ strlen(menu_2[choose_pos_2]) + 3 + 1, choose_pos_2 };
            SetConsoleCursorPosition(hStdOut, cursorPos);
            printf(">>");
            iKey_2 = _getch();
        }
...
int main() {
    ...
    int size;
    int* mass;
    printf("Добро пожаловать. В данном меню вы сможете выполнить некоторые действия над массивами");
    printf("\n\nВведите размер массива, который хотите рассмотреть - ");
    scanf_s("%d", &size);
